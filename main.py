...
# 제목과 설명
st.title("🎬 MBTI로 알아보는 과학·수학 명작 영화")
st.markdown("당신의 성격에 딱 맞는 💡지적이고 🎇흥미로운 영화들을 추천해드릴게요!")

# 선택 메뉴
mbti_options = [
    "선택 안 함", "INTJ", "INFP", "ENTP", "ISFJ", "ESTJ", "INFJ", "ENFP", "ISTP"
]
mbti = st.selectbox("📌 당신의 MBTI를 선택해주세요!", mbti_options)
...
if mbti != "선택 안 함":
    ...
    for movie in recommendations.get(mbti, []):
        st.markdown(f"- {movie}")
